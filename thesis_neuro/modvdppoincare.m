...
function modvdppoincare(a,b,eps,N)
figure(20); figure(30);
close([20 30]);
...
% Attracting manifold
for i=1:N
    [t,x1]=ode45(@(t,x) modvdp(t,x,eps,a,b),tspan,shootatt(:,i)',options); %Flow fwd
    [t,x2]=ode45(@(t,x) modvdp(t,x,eps,a,b),tspanplus,x1(end,:),options); %Flow fwd
    x=[x1; x2];
    att(i,:,:)=x; 
...
for i=1:N
    % Flow back
    [t,x1]=ode45(@(t,x) backmodvdp(t,x,eps,a,b),tspan,shootrep(:,i)',options); 
%    [t,x2]=ode45(@(t,x) backmodvdp(t,x,eps,a,b),tspan,x1(end,:),options);
%    x=[x1; x2];
    x=x1;
    rep(i,:,:)=x;
...
figure(30)
%plot(attplot(:,1),attplot(:,2),'Color','b');
%hold on;    
%plot(repplot(:,1),repplot(:,2),'Color','r');
plot1=plot([attplot(:,1) repplot(:,1)],[attplot(:,2) repplot(:,2)]);
%plot1=plot([angles' angles'],[attplot(:,2) repplot(:,2)]);
set(plot1(1),'Color','b','DisplayName','Attracting Manifold');
set(plot1(2),'Color','r','DisplayName','Repelling Manifold');
...
%% modvdp
function xdot=modvdp(t,x,eps,a,b)
...
xdot(1) = (x(3) - (2*(x(1).^2 + x(2).^2).^(3/2) - 3*(x(1).^2 + x(2).^2) + 1)).*x(1) - 5*x(2)*(x(1)^2+x(2)^2); 
xdot(2) = (x(3) - (2*(x(1).^2 + x(2).^2).^(3/2) - 3*(x(1).^2 + x(2).^2) + 1)).*x(2) + 5*x(1)*(x(1)^2+x(2)^2); 
xdot(3) = eps.*(a - ((x(1) - b).^2 + x(2).^2).^(1/2));

xdot=xdot';

%% backward modvdp
function xdot=backmodvdp(t,x,eps,a,b)
% Differential equations for rotating van der Pol system
% Used by Barry, Benes, Burke, Kaper, and Kramer
% Based on rotating the van der Pol oscillator about the z-axis
% x(1)=x, x(2)=y, x(3)=z
