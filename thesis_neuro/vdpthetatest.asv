...
function vdpthetatest(a,b,eps,thetadot,N)
%figure(20); figure(30);
close([30 40 50]);
global tdot
...
% Attracting manifold
for i=1:N
    [t,x1]=ode45(@(t,x) rotvdp(t,x,eps,a,b),tspan,shootatt(:,i)',options); %Flow fwd
    [t,x2]=ode45(@(t,x) rotvdp(t,x,eps,a,b),tspanplus,x1(end,:),options); %Flow fwd
    x=[x1; x2];
    att(i,:,:)=x; 
...
for i=1:N
    % Flow back
    [t,x1]=ode45(@(t,x) backrotvdp(t,x,eps,a,b),tspan,shootrep(:,i)',options); 
...
title({'Intersections of attracting and repelling branches with the cylinder r=1';...
    ['(a = ',num2str(a),', b = ',num2str(b),', \epsilon = ',num2str(eps),', \omega = ',num2str(tdot),')']});
legend('Location','Best');
axis([-pi pi -.0295 -.0045]);
...
for i=1:N
    figure(50)
    [t,x1]=ode45(@(t,x) rotvdp(t,x,eps,a,b),tspan,att(i,end,:),options); %Flow fwd
    traj(:,:)=[squeeze(att(i,:,:)); x1]; 
    rad=(traj(:,1).^2+traj(:,2).^2).^(1/2); %compute radial coord
    plot(rad(:)',traj(:,3),'Color',colors(i,:));
    hold on;    
end
title({[num2str(N-1),' trajectories in the r-z plane'];...
    ['(a = ',num2str(a),', b = ',num2str(b),', \epsilon = ',num2str(eps),', \omega = ',num2str(tdot),')']});
...
xdot(1) = (x(3) - (2*(x(1).^2 + x(2).^2).^(3/2) - 3*(x(1).^2 + x(2).^2) + 1)).*x(1) - tdot*x(2); 
xdot(2) = (x(3) - (2*(x(1).^2 + x(2).^2).^(3/2) - 3*(x(1).^2 + x(2).^2) + 1)).*x(2) + tdot*x(1); 
